...
```{r load packages, eval=F,echo=T}
  ...
  required.packages <- c("data.table","doSNOW","ERAg","ncdf4","raster","rgeos","sp","terra")
...
ReformatCHIRPS<-function(CHIRPS_dir="CHIRPS Downloads"),
...
#### Add altitude
### POWER
Before we can use `ExtractPOWER` or calculate PET we need to add altitude data to ERA. Altitude data can be added from the `ERA_Physical` object of the `ERAg` package.

```{r Add Altitude,eval=F}
...
Agroclimatology datasets such as POWER contain the variables needed to calculate reference PET, but before we can do this we need to gap fill some missing solar radiation values.  
